...
// @downloadURL  https://github.com/SlavaSt295/Blum/blob/main/blum-autoclicker-slv-3.17.js
// @updateURL    https://github.com/SlavaSt295/Blum/blob/main/blum-autoclicker-slv-3.17.js
// @homepage     https://github.com/mudachyo/Blum
// ==/UserScript==

const SCRIPT_VERSION = '3.17';
const SCRIPT_URL = 'https://github.com/SlavaSt295/Blum/raw/refs/heads/main/blum-autoclicker-slv-3.17.user.js';
...
const generateGameSettings = () => {
    ...
    return {
        ...
        flowerSkipPercentage: Math.floor(Math.random() * 11) + 15,
	    minDelayMs: Math.floor(Math.random() * (550 - 450 + 1)) + 450, // 450 - 550
	    maxDelayMs: Math.floor(Math.random() * (999 - 800 + 1)) + 800, // 800 - 999
    }
}

GAME_SETTINGS = { ...GAME_SETTINGS, ...generateGameSettings() };
// slv
...
try {
    ...
    function checkGameCompletion() {
        ...
        if (rewardElement && !gameStats.isGameOver) {
            ...
            document.querySelector('body').dispatchEvent(new Event('gameFinished'));
        }
    }
	...
	function answerQuestion() {
		console.log("answerQuestion ", new Date().toISOString());
		try {
	...
	// slv
	setInterval(answerQuestion, Math.random() * (7120 - 3125) + 3125);
	function clickWithDelay(buttons) {
	    const clickButtonWithDelay = (index) => {
	        if (index >= buttons.length) return;

	        buttons[index].click();
	        const delay = Math.random() * (1500 - 900) + 900;

	        setTimeout(() => clickButtonWithDelay(index + 1), delay);
	    };
	...
	function videoTaskCompletion() {
		console.log("videoTaskCompletion ", new Date().toISOString());
		if (isGameToolPaused) return;
		...
		// }
	    if (GAME_SETTINGS.autoVerifyCode) {
	        const verifyButtons = document.querySelectorAll(".tasks-list button[class*=not-started]");
	        clickWithDelay(Array.from(verifyButtons));
	    }
	    if (GAME_SETTINGS.autoClaimTask) {
	        const claimButtons = document.querySelectorAll("button.is-status-ready-for-claim");
	        clickWithDelay(Array.from(claimButtons));
	    }
		// slv
	}

	// slv
	setInterval(videoTaskCompletion, Math.random() * (7120 - 3125) + 3125);
